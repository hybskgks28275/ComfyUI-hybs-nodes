...
const CT_INT_LIST = "HYBS_IntList";
...
function getValues(node) {
  return findValueWidgets(node).map((widget) => toInteger(widget.value));
}
...
function addValueWidget(node, selectionWidget, value = 1) {
  ...
  const widget = node.addWidget(
    ...
    toInteger(value),
    () => {
      const intValue = toInteger(widget.value);
      if (widget.value !== intValue) {
        widget.value = intValue;
      }
      storeSelection(node, selectionWidget);
      app.graph?.setDirtyCanvas?.(true, true);
    },
    { min: -2147483648, max: 2147483647, step: 1, precision: 0 },
  );
...
function refreshFromSelection(node, selectionWidget, mode = "preserve") {
  ...
  let values = [];
  try {
    values = JSON.parse(selectionWidget.value || "[1]");
  } catch (_error) {
    values = [1];
  }
...
function addValue(node, selectionWidget) {
  const baseSize = Array.isArray(node.size) ? [node.size[0], node.size[1]] : null;
  removeControlWidgets(node);
  addValueWidget(node, selectionWidget, 1);
  addControlWidgets(node, selectionWidget);
  storeSelection(node, selectionWidget);
